...
export const Resultados = () => {
    ...
    const getBackendData = async () => {
      if (playas === undefined) {
        let response = await fetch(
          URL_BACKEND +
            `?latitud=${latitud}&longitud=${longitud}&dia=${dia}&hora=${hora}`
        );
        ...
        <Row className="px-5 justify-content-center">
          <Col lg={4}>
            <h3 class="text-center">
              Sentímolo moito, tivo lugar un erro cargando a páxina.{" "}
            </h3>
          </Col>
        </Row>
  ...
  const destino = {
    lat: playas[rankingPlayas[playaActiva].nombre].lat,
    lng: playas[rankingPlayas[playaActiva].nombre].lon,
  };
  ...
  const createRouting = (props) => {
    ...
    instance.on("routesfound", function (routeResult) {
      setDistancia(
        Math.round(routeResult.routes[0].summary.totalDistance / 1000)
      );
            ...
            {rankingPlayas.slice(0, 5).map((playa, index) => {
              return (
                <ListGroup.Item
                  ...
                  className={
                    index === playaActiva ? "text-white bg-primary" : ""
                  }
                  onClick={() => {
                    setPlayaActiva(index);
                  }}
                  key={index}
          ...
          <Row className="p-2 border border-secondary shadow border-2 rounded">
            <Col md={5}>
              <h4 className="text-center">
                <u>{rankingPlayas[playaActiva].nombre}</u>
              </h4>
              <p>
                {"Vento: " +
                  playas[rankingPlayas[playaActiva].nombre].viento +
                  " Km/h"}
              </p>
              <p>
                {"Temperatura: " +
                  playas[rankingPlayas[playaActiva].nombre].temperatura +
                  " ºC"}
              </p>
              <p>
                {"Cobertura de nubes: " +
                  playas[rankingPlayas[playaActiva].nombre].coberturaNubes +
                  " %"}
              </p>
